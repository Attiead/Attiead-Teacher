package com.attiead.teacher.attieadteacher.common.filter

import com.attiead.teacher.attieadteacher.common.logback.HttpLog
...
class LoggingFilter : OncePerRequestFilter() {
    ...
    private fun loggingHttp(
        ...
        log[HttpLog.URL] = request.requestURI
        ...
        if (hasPayload(request)) {
            ...
            log[HttpLog.REQUEST] = attieadLogger.jsonLogFormat(requestWrapper.contentAsByteArray)
        }
        if (isSupportedMediaType(responseWrapper.contentType)) {
            log[HttpLog.RESPONSE] = attieadLogger.jsonLogFormat(responseWrapper.contentAsByteArray)
        }
        log[HttpLog.STATUS] = responseWrapper.status
        responseWrapper.copyBodyToResponse()
        attieadLogger.info(log)
    }
